	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
		...
		http.antMatcher("/**")
				...
				.authorizeRequests()
					...
					.antMatchers("/inscription","/login").anonymous() //anonymous es lo contrario a authenticated, on est obligés d'etre connectés
					.anyRequest().permitAll()  //solo lo que ya esta autenticado?
					.and()
					.formLogin()
					.and()
					.logout(); //nos va a mandar a produit despues de desconectarse
		// @formatter:on
		return http.build();
	}
